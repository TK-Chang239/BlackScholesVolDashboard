...
CAPTIONS = {
    ...
    "P8a": (
        ...
        "histogram below this line also includes sessions the archive holds no "
        "quote for, where the straddle is marked with our own model at the last "
        "quoted vol. The thin grey lines are the individual trades; the blue "
        "line is their sum."
    ),
    "P8b": (
    ...
    "P8c": (
        "The same P&L, one session at a time — counting only the sessions the "
        "archive actually quotes. A day the chain does not cover is marked with "
        "our own model at the last quoted vol, so it moves with spot and time "
        "decay alone and is Black-Scholes-conforming by construction; letting "
        "those days in would narrow this histogram towards the answer the model "
        "wants. Each trade's opening day is dropped too — it is a definitional "
